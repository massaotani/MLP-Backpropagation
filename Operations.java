...
public class Operations {
    ...
    public static Double backPropagation(Double[][] saida, Double[][] oculta, Double[][] entrada, Double[][] esperado, RedeNeural rede) {
        ...
        Double[][] deltaSaida = Matrix.ProdutoHadamard(sigmoideDerivada, matrizDeErros); //delta "puro" (sem taxa de aprendizado), usado tambem para propagar o erro para a camada oculta
        ...
        Double[][] infoErroSaida = Matrix.MultiplyEscalar(deltaSaida, rede.getTaxaAprendizado());
        ...
        // TRANSPOSTA DA OCULTA (usa os pesos ANTIGOS, de antes da atualizacao acima -
        // o backpropagation precisa propagar o erro pelos pesos que estavam em vigor
        // durante o feedforward, nao pelos que acabaram de ser corrigidos)
        Double[][] pesosHiddenTransposta = Matrix.transposta(pesosOcultaAntigos);

        //Multiplica W pela taxa de Erro para calcular a taxa de erro da camada oculta
        //Delta INj = Matriz W * Delta K (delta "puro" da saida, ja com F'(Y_INk) aplicado)
        Double[][] errosOculta = Matrix.Multiply(pesosHiddenTransposta, deltaSaida, null);
